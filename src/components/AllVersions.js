...
const AllVersions = ({ product }) => {
  ...
  return (
    <div className="docs__container docs__all-versions">
      <h1 className="docs__heading-primary">{product.name} documentation by version</h1>
      ...
      <ul className="docs__all-versions-list">
        {product.version.filter(v => !v.archived).map(ver => (
          <li key={ver.id}><Link to={`/docs/${product.slug}/${ver.link}`}>{product.name} {ver.name}</Link></li>
        ))}
      </ul>
      {archivedVersions.length > 0 && <>
        ...
        <ul className="docs__all-versions-list">
          {product.version.filter(v => v.archived).map(ver => (
            <li key={ver.id}><a href={`${archivedDocsUrl}/docs/${product.slug}/${ver.link}`}>{product.name} {ver.name}</a></li>
          ))}
        </ul>
      </>}
    </div>
