...
In these cases, you should use the [never-proxy](../cluster-config#neverproxy) configuration to prevent certain
...
cluster. In the values file that you pass to `telepresence helm install [--upgrade] --values <values file>`, add a `client.routing`
entry like so:
