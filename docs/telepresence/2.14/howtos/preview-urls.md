...
3. Start the intercept with `telepresence intercept <service-name> --port <TCP-port> --env-file <path-to-env-file> --mechanism http`and adjust the flags as follows:
 Start the intercept:
 ...
 * You can remove the **--mechanism http** flag if you have your traffic-manager set to *team-mode*
...
4. Answer the question prompts.
   ...
   ```console
$ telepresence intercept example-service --mechanism http --ingress-host ambassador.ambassador --ingress-port 80 --ingress-l5 dev-environment.edgestack.me --ingress-tls --port 8080 --env-file ~/ex-svc.env
