...
or externally by creating an `AmbassadorMapping` similar to below:
...
that is handling traffic for a given `AmbassadorMapping`.  The name of a given
...
Now, create a service and `AmbassadorMapping` to expose Grafana behind
$productName$:
