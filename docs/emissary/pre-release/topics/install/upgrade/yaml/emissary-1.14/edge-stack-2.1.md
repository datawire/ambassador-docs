...
<Alert severity="warning">
  ...
  If you originally installed with Helm, see the <a href="../../../helm/emissary-1.14/edge-stack-2.1">Helm-based
...
3. **Install $AESproductName$ $version$.**
   ...
   **in the same namespace as your existing $productName$ 1.14.2 installation**. It's important
   ...
   Our `aes-defaultns-migration.yaml` assumes that $productName$ 1.14.2 is installed in the
   `default` namespace. If you installed $productName$ 1.14.2 into a different namespace, you'll
   need to download `aes-defaultns-migration.yaml` file and edit it.
   
   **If you need to set `AMBASSADOR_LABEL_SELECTOR`**, download `aes-defaultns-migration.yaml` and edit it to
   do so.
