# The `AmbassadorHost` CRD, ACME support, and external load balancer configuration
...
The `AmbassadorHost` is responsible for high-level TLS configuration in $productName$.

The are two settings in the `AmbassadorHost` that are responsible for TLS configuration:
...
Regardless of if you are using the built in ACME support in the $AESproductName$, the `AmbassadorHost` is responsible for reading TLS certificates from Kubernetes
...
certificate with ACME by setting `acmeProvider.authority: none` in the `AmbassadorHost`.
...
- [The `AmbassadorHost` CRD, ACME Support, and External Load Balancer Configuration](#the-host-crd-acme-support-and-external-load-balancer-configuration)
  ...
  - [Load Balancers, the `AmbassadorHost` Resource, and `X-Forwarded-Proto`](#load-balancers-the-host-resource-and-x-forwarded-proto)
...
### HTTP-only
  ...
  In this case, the `AmbassadorHost` resource explicitly requests no ACME handling and no TLS, then states that insecure requests must be routed instead of redirected.
...
### L7 LB
  ...
  If using an L7 LB, **we recommend that the LB handle TLS termination and redirection of cleartext**. For this use case, you can use an `AmbassadorHost` without TLS, but still turn on redirection as a failsafe:
...
The `AmbassadorHost` CRD defines how $productName$ will be visible to the outside world. A minimal `AmbassadorHost` defines a hostname by which $productName$ will be reachable, but an `AmbassadorHost` can also tell $productName$ how to manage TLS, and which resources to examine for further configuration.
