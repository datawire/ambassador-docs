...
> Note that when setting `retry_policy`, adjusting `max_retries` in the [circuit breaker](https://www.getambassador.io/docs/edge-stack/2.0/topics/using/circuit-breakers/) configuration should also be considered in order to account for all desired retries.
