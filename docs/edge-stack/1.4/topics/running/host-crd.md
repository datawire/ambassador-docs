...
* [HTTP-only](#http-only)
* [L4 LB, HTTPS-only, TLS terminated at Ambassador, not redirecting cleartext](#l4-lb-https-only-tls-terminated-at-productname-not-redirecting-cleartext)
...
### L4 split LB, TLS terminated at Ambassador
  ...
  Configure this exactly like [case 2](#https-only-tls-terminated-at-productname-redirecting-cleartext-from-port-8080). Leave `xff_num_trusted_hops` in the `ambassador` module at its default of 0.
