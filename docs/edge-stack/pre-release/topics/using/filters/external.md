...
* In an `AuthService`, the `tls` field may either be a Boolean, or a
  string referring to a `TLSContext`.  In an `External` `Filter`, it
  may only be a Boolean; referring to a `TLSContext` is not supported.
* In an `AuthService`, the `add_linkerd_headers` field defaults based
  on the [`ambassador` `Module`](../../../running/ambassador).  In an
